...
class DataController:
    ...
    def save_shipment(self, shipment, preserve_box_items=False) -> bool:
        ...
        try:
            ...
            logger.info(f"Сохранение {len(shipment.boxes)} коробок для поставки {shipment.destination_name}")
            for i, box in enumerate(shipment.boxes):
                is_current = 1 if i == shipment.current_box_index else 0
                # box_id уже строка, не нужно кодировать
                box_id_str = box.box_id if isinstance(box.box_id, str) else str(box.box_id)
                logger.info(f"Коробка {i}: box_id={box_id_str}, is_current={is_current}, items_count={len(box.items)}")
            ...
            # Теперь нужно получить ID коробок и вставить элементы коробок
            # Это выполняется в отдельной транзакции для каждой коробки
            for i, box in enumerate(shipment.boxes):
                box_id_str = box.box_id if isinstance(box.box_id, str) else str(box.box_id)
                box_result = execute_query(
                    f"""
                    SELECT id FROM boxes WHERE shipment_id = {placeholder} AND box_id = {placeholder}
                    """,
                    (shipment_id, box_id_str),
                    fetchone=True
                )
                ...
                if box_db_id:
                    logger.info(f"Сохранение товаров для коробки {box_id_str} (box_db_id={box_db_id}): {len(box.items)} товаров")

                    # Собираем запросы для транзакции
                    box_queries = []
                    
                    # Если не нужно сохранять содержимое коробок, удаляем существующие элементы
                    ...
                    # Вставляем элементы коробки
                    for barcode, qty in box.items.items():
                        if preserve_box_items:
                            # Проверяем, существует ли уже такой элемент в коробке
                            existing_result = execute_query(
                                f"SELECT id FROM box_items WHERE box_id = {placeholder} AND barcode = {placeholder}",
                                (box_db_id, barcode),
                                fetchone=True
                            )

                            if existing_result:
                                # Если элемент уже существует, обновляем количество (заменяем, а не добавляем)
                                box_queries.append((
                                    f"UPDATE box_items SET qty = {placeholder} WHERE box_id = {placeholder} AND barcode = {placeholder}",
                                    (qty, box_db_id, barcode)
                                ))
                            else:
                            ...
                            # Перезаписываем при отсутствии preserve_box_items
                            if use_sqlite:
                                box_queries.append((
                                    f"""
                    ...
                    # Выполняем все запросы для коробки в одной транзакции
                    if box_queries:
                        try:
                            execute_transaction(box_queries)
                        except Exception as e:
                    ...
                    logger.info(f"Сохранено {len(box.items)} товаров в коробку {box_id_str}")
            
            logger.info(f"Поставка успешно сохранена: {shipment.destination_name}")
            return True
